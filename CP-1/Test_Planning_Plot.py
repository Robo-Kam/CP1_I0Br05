...
class Grid:
    ...
    def _next_position(position, goal):
        """Allows the robot to change its next move. Replace with A* Method"""
        row, column = position
...
def create_grid(grid_size=10, robot_count=10):
    grid = Grid(grid_size)
...
def plot_grid(grid):
    ...
    def planned_path(new_robot):
        """Plot method from A* Method"""
        path = list(new_robot.path or [])
        if not path or path[0] != new_robot.position:
            path.insert(0, new_robot.position)
        if path[-1] != new_robot.goal:
            path.append(new_robot.goal)
        return path
    ...
    def draw_robot_grid():
        ...
        axis.set_title(f"Robot {robot_num + 1}: {new_robot.name}")
